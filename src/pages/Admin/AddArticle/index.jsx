...
const AddArticle = props => {
  ...
  const updateFromDB = (dbName, post) => {
    const page = dbName === 'articles' ? '/admin/article' : '/admin/draft';
  ...
  const turnDraft = () => {
    ...
    if (!isEdit) {
      // 新建文章按钮进来的
      addToDB('articles', false);
    } else {
      // 修改按钮进来的
      if (isDraft) {
        // 草稿页的修改按钮
        updateFromDB('articles', false);
      } else {
        // 文章页的修改按钮
        updateFromDB('articles', false);
        // oldCount--
